...
void DisplayAnimation(FbxScene* pScene)
{
    ...
    for (i = 0; i < pScene->GetSrcObjectCount<FbxAnimStack>(); i++)
    {
        ...
        // ===============================
        // 从根节点开始，递归遍历整棵 Node 树
        // 在遍历过程中，查找每个节点上的动画曲线
        //
        // 注意：
        // 这里的 DisplayAnimation 不是上面这个函数，
        // 而是另一个重载版本：
        // DisplayAnimation(FbxAnimStack*, FbxNode*)
        // ===============================
...
/// //////////////////////////////////////////////////////////
/// </summary>
void DisplayAnimation(FbxAnimStack* pAnimStack, FbxNode* pNode, FbxTime t, bool isSwitcher = false);
void DisplayAnimation(FbxAnimLayer* pAnimLayer, FbxNode* pNode, FbxTime t, bool isSwitcher = false);
void DisplayChannels(FbxNode* pNode, FbxAnimLayer* pAnimLayer, void (*DisplayCurve) (FbxAnimCurve* pCurve), void (*DisplayListCurve) (FbxAnimCurve* pCurve, FbxProperty* pProperty), FbxTime t, bool isSwitcher);
void DisplayAnimation(FbxScene* pScene, FbxTime t)
{

    for (int i = 0; i < pScene->GetSrcObjectCount<FbxAnimStack>(); i++)
    {
        FbxAnimStack* lAnimStack = pScene->GetSrcObject<FbxAnimStack>(i);
        DisplayAnimation(lAnimStack, pScene->GetRootNode(),t);
    }
}
void DisplayAnimation(FbxAnimStack* pAnimStack, FbxNode* pNode, FbxTime t, bool isSwitcher)
{
    int l;
    ...
    for (l = 0; l < nbAnimLayers; l++)
    {
        ...
        DisplayAnimation(lAnimLayer, pNode, t, isSwitcher);
    }
}
void DisplayAnimation(FbxAnimLayer* pAnimLayer, FbxNode* pNode, FbxTime t, bool isSwitcher)
{
    int lModelCount;

    DisplayChannels(pNode, pAnimLayer, DisplayCurveKeys, DisplayListCurveKeys, t, isSwitcher);
...
void DisplayChannels(FbxNode* pNode, FbxAnimLayer* pAnimLayer, void (*DisplayCurve) (FbxAnimCurve* pCurve), void (*DisplayListCurve) (FbxAnimCurve* pCurve, FbxProperty* pProperty), FbxTime t, bool isSwitcher)
{
    ...
    //t1.SetFrame(10, FbxTime::eFrames30);

    FbxAMatrix m0 = pNode->EvaluateGlobalTransform(t);

    FbxVector4 T0 = m0.GetT();
    FbxVector4 R0 = m0.GetR();
    FbxVector4 S0 = m0.GetS();
...
void DisplayChannels(FbxNode* pNode, FbxAnimLayer* pAnimLayer, void (*DisplayCurve) (FbxAnimCurve* pCurve), void (*DisplayListCurve) (FbxAnimCurve* pCurve, FbxProperty* pProperty), bool isSwitcher)
{
    ...
    if (!isSwitcher)
    {
        ...
        //auto tx = pNode->LclTranslation.GetCurve(pAnimLayer, FBXSDK_CURVENODE_COMPONENT_X,true);
        //auto rx = pNode->LclRotation.GetCurve(pAnimLayer, FBXSDK_CURVENODE_COMPONENT_X);
        //std::cout << "TX\n";
        //if (tx) std::cout << "TX has curve\n";
        //if (rx) std::cout << "RX has curve\n";

        bool hasCurve = false;

        if (pNode->LclTranslation.GetCurve(pAnimLayer, FBXSDK_CURVENODE_COMPONENT_X)) hasCurve = true;
        if (pNode->LclRotation.GetCurve(pAnimLayer, FBXSDK_CURVENODE_COMPONENT_X))    hasCurve = true;
        if (pNode->LclScaling.GetCurve(pAnimLayer, FBXSDK_CURVENODE_COMPONENT_X))     hasCurve = true;
        ...
        FbxAMatrix m0 = pNode->EvaluateGlobalTransform(t0);
        FbxAMatrix m1 = pNode->EvaluateGlobalTransform(t1);

        FbxVector4 T0 = m0.GetT();
        FbxVector4 R0 = m0.GetR();
        FbxVector4 S0 = m0.GetS();

        FbxVector4 T1 = m1.GetT();
        FbxVector4 R1 = m1.GetR();
        FbxVector4 S1 = m1.GetS();
