...
{
    return keystates[keycode] && !old_keystates[keycode];
...
bool Keyboard::KeyJustReleased(unsigned char keycode) noexcept
{
    return !keystates[keycode] && old_keystates[keycode];
}

void Keyboard::UpdateState()noexcept {
    old_keystates = keystates;
}
...
bool Keyboard::AnyKeyIsPressed() const noexcept
{
    return keystates.any(); // 1つでも押されていればtrue
}

bool Keyboard::NoKeyIsPressed() const noexcept
{
    return keystates.none(); // std::bitset の none() で全ビットが0か確認
}
